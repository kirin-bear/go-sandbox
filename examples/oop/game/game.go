...
func CastTo(spell Spell, object any) {
	...
	//log.Println("Cast " + spell.Name() + " " + spell.Char() + " object " + reflect.ValueOf(object).Type().String())
	//log.Println(reflect.TypeOf(object).String())

	if _, ok := object.(Wall); ok{
		return
	}

	if castReceiver, ok := object.(CastReceiver); ok {
		castReceiver.ReceiveSpell(spell)
		return
	}
	...
	health := val.FieldByName("Health")
	if health.IsValid() {
		//log.Println("Да можно изменить")
        if health.CanSet() {
			//log.Println("Да можно изменить")
			if (spell.Value() > 0) {
				health.SetInt(health.Int() + int64(spell.Value()))
			} else {
				health.SetInt(health.Int() - int64(spell.Value()))
			}
		}
